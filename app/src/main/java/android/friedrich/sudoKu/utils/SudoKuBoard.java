...
public class SudoKuBoard {
    ...
    private AssignmentTracker mAssignmentTracker;
    ...
    public SudoKuBoard() throws Exception {
        ...
        mAssignmentTracker = new AssignmentTracker();
//        setUnits();
//        setPeers();
    }

    public void bindAssignmentTracker(AssignmentTracker tracker) {
        mAssignmentTracker = tracker;
    }
...
//        mAssignmentTracker.addRecord(cellIndex,Byte.parseByte(String.valueOf(number)));
//        int trackerIndex = mAssignmentTracker.getSize();
        for (int i = 0; i < otherValues.length(); i++) {
            int trackerIndex = mAssignmentTracker.getSize();
            if (!eliminate(possibleValues, cellIndex, /* number need to remove*/otherValues.charAt(i))) {
//                while (mAssignmentTracker.getSize() > trackerIndex) {
//                    mAssignmentTracker.removeRecord();
//                }
                mAssignmentTracker.rollback(trackerIndex);
                return false;
            }
        }
//        mAssignmentTracker.addRecord(cellIndex, Byte.parseByte(String.valueOf(number)));
...
//                mAssignmentTracker.removeRecord();
                ...
                Set<Integer> peers = getGridPeers(cellIndex);
                mAssignmentTracker.addRecord(cellIndex, Byte.parseByte(values));
                for (Integer x :
                        peers) {
...
//                        mAssignmentTracker.removeRecord();
            ...
            maybe occur.
             */
            trackerIndex = mAssignmentTracker.getSize();
...
//                    mAssignmentTracker.removeRecord();
                     ...
                     */
//                    mAssignmentTracker.addRecord(numberEnsureIndex,Byte.parseByte(String.valueOf(number)));
                    int trackerIndexInloop = mAssignmentTracker.getSize();
                    if (!assignValue(possibleValues, numberEnsureIndex, number)) {
//                        mAssignmentTracker.removeRecord();
                        mAssignmentTracker.rollback(trackerIndexInloop);
                        return false;
                    }
                }
            }
...
//                    Log.i(TAG, String.format("search: assign %d to index %c failed",
//                            indexOfFirstAssignCell, copy[indexOfFirstAssignCell].charAt(i)));
//                    while (mAssignmentTracker.getSize() > trackerIndexInloop) {
//                        mAssignmentTracker.removeRecord();
//                    }
                    mAssignmentTracker.rollback(trackerIndexInloop);
                }
...
//        String squareName = sSquareNames[index];
    ...
    public byte getNumber(int index) {
        return Byte.parseByte(mPossibleValues[index]);
    }
}
